 ...
 * <h2>覆盖</h2>
 * <ul>
 *   <li>TC-JC-01: {@link ToStringSerializer} 注册到 Long 与 long (防止 19 位 Snowflake ID 精度丢失)</li>
 ...
 * 必须 Long → JSON string。</p>
 *
 * <h2>为什么不使用 {@code @JsonTest}</h2>
 * <p>{@code @JsonTest} 会触发 Spring Boot 最小上下文加载, 而
 * {@code spring-cloud-commons 2023.x} 的
 * {@code ConfigDataMissingEnvironmentPostProcessor} 在
 * {@code @EnableAutoConfiguration} 存在时强制要求 {@code spring.config.import},
 * 否则抛 {@code ImportException}。本测试不依赖 Spring 上下文,
 * 用 {@link JsonMapper} 直接应用 {@link JacksonConfig#longAsStringCustomizer}
 * 的等价配置 — 行为相同, 启动开销为零。</p>
 */
...
class JacksonConfigTest {
    ...
    /**
     * 应用 JacksonConfig 的等价 serializer 注册, 直接构造 ObjectMapper
     * (等价 {@code JacksonConfig.longAsStringCustomizer().customize(builder).build()})
     */
    ...
    void writeLongAsStringFeature_enabled() throws JsonMappingException {
    ...
    void longField_serializedAsString() throws JsonProcessingException {
        ObjectMapper objectMapper = buildObjectMapper();
        ...
        // 期望 "id":"2069257993972027393" (string), 不是 2069257993972027400 (丢精度数字)
        assertThat(json)
                .contains("\"id\":\"2069257993972027393\"")
                .contains("\"parkId\":\"2069257993972027393\"");
    }

    @Test
    @DisplayName("TC-JC-03: String / Integer 字段不受影响 (按原类型输出)")
    void otherTypes_unaffected() throws JsonProcessingException {
        ObjectMapper objectMapper = buildObjectMapper();
        SnowflakeEntity entity = new SnowflakeEntity();
        entity.setId(SNOWFLAKE_ID);
        ...
        String json = objectMapper.writeValueAsString(entity);
        assertThat(json).contains("\"name\":\"A 座\"");
        // floorNumber 是 Integer, 不受 WRITE_LONG_AS_STRING 影响, 仍输出为 number
        assertThat(json).contains("\"floorNumber\":10");
    }
