...
const defaultForm = { typeName: '', typeDescribe: '', status: 1 }
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value && currentId.value) {
      ...
      if (res.code === 200) { ElMessage.success('新增成功'); dialogVisible.value=false; loadData() } else ElMessage.error(res.message||'更新失败')
    }
