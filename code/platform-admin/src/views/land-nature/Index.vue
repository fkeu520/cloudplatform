...
const defaultForm = { parkId: 1, landNatureName: '', landNatureCode: '', color: '#909399', status: 1 }
const formData = reactive({ ...defaultForm })
const rules = {
  parkId: [{ required: true, message: '请输入园区ID', trigger: 'blur' }],
  landNatureName: [{ required: true, message: '请输入土地性质名称', trigger: 'blur' }]
}
...
async function handleEdit(row: any) {
  ...
  try { const res: any = await getLandNatureById(row.id); if (res.code === 200) Object.assign(formData, res.data) } catch { ElMessage.error('获取详情失败') }
