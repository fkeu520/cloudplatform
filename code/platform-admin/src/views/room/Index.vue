...
import { getRoomPage, getRoomById, createRoom, updateRoom, deleteRoom, updateRoomStatus } from '@/api/room'
import { getParkList } from '@/api/park'
import { getBuildingPage } from '@/api/building'

const loading = ref(false); const tableData = ref<any[]>([]); const total = ref(0)
const pageNum = ref(1); const pageSize = ref(10)
const searchForm = reactive({ keyword: '', roomType: undefined as string | undefined, status: undefined as number | undefined })
...
const defaultForm = { parkId: undefined as number | undefined, buildingId: undefined as number | undefined, floor: 1, roomNo: '', roomType: 'OFFICE', area: undefined as number | undefined, monthlyRent: undefined as number | undefined, remark: '' }
...
function roomStatusLabel(s: number): string { return ['空置', '已租', '装修中', '停用'][s] || '-' }
function roomStatusTagType(s: number): string { return ['success', 'primary', 'warning', 'info'][s] || '' }
...
function handleReset() { searchForm.keyword = ''; searchForm.roomType = undefined; searchForm.status = undefined; handleSearch() }
...
async function handleEdit(row: any) {
  resetForm(); isEdit.value = true; currentId.value = row.id; dialogTitle.value = '编辑房源'
  try { const res: any = await getRoomById(row.id); if (res.code === 200) Object.assign(formData, res.data) } catch { ElMessage.error('获取房源详情失败') }
...
async function handleUpdateStatus(row: any) {
  try {
    const { value: status } = await ElMessageBox.prompt('输入新状态: 0=空置 1=已租 2=装修中 3=停用', '状态变更', {
      inputValue: String(row.status ?? 0), inputValidator: (v: string) => ['0', '1', '2', '3'].includes(v) || '请输入 0/1/2/3'
    })
