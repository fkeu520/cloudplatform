  ...
  <div class="page-container">
    <el-card class="search-card">
      ...
      <el-alert type="info" :closable="false" style="margin-bottom: 16px">
        锁定/解锁历史记录（append-only），只能新增和查询，不能修改或删除
      </el-alert>
    </el-card>
...
<el-table-column label="房间" width="150"><template #default="scope">{{ roomMap[scope.row.roomId] || scope.row.roomId }}</template></el-table-column>
        ...
        <el-table-column prop="enterpriseId" label="客户ID" width="100" />
        <el-table-column prop="enterpriseName" label="客户名称" min-width="150" :show-overflow-tooltip="true" />
        <el-table-column prop="operator" label="操作人" width="120" />
        <el-table-column prop="reason" label="原因" min-width="200" :show-overflow-tooltip="true" />
        <el-table-column prop="days" label="锁定天数" width="100" align="right" />
    ...
    <el-dialog v-model="dialogVisible" title="新增锁定记录" width="600px" @close="resetForm">
      <el-form :model="formData" label-width="100px" :rules="rules" ref="formRef">
        ...
        <el-form-item label="客户ID"><el-input-number v-model="formData.enterpriseId" :min="0" style="width:100%" /></el-form-item>
        <el-form-item label="客户名称"><el-input v-model="formData.enterpriseName" maxlength="128" /></el-form-item>
        <el-form-item label="操作人"><el-input v-model="formData.operator" maxlength="64" /></el-form-item>
        <el-form-item label="锁定天数"><el-input-number v-model="formData.days" :min="0" style="width:100%" /></el-form-item>
        <el-form-item label="原因"><el-input v-model="formData.reason" type="textarea" :rows="2" maxlength="500" /></el-form-item>
      </el-form>
      <template #footer>
...
async function loadData() {
  loading.value = true
  try {
    const res: any = await getRoomLockRecordPage({ ...searchForm, pageNum: pageNum.value, pageSize: pageSize.value })
    if (res.code === 200) { tableData.value = res.data.records || []; total.value = res.data.total || 0 }
  } finally { loading.value = false }
}
function handleSearch() { pageNum.value = 1; loadData() }
function handleReset() { searchForm.roomId=undefined; searchForm.parkId=undefined; searchForm.isLock=undefined; handleSearch() }
...
async function handleSubmit() {
  ...
  try {
    const res: any = await createRoomLockRecord(formData)
