...
#
# 用法:
#   ./scripts/diag/apply-v46-on-217.sh
#
# 前置:
...
SQL_FILE="${SCRIPT_DIR}/code/platform-server/park-space/src/main/resources/db/migration/V46__ensure_room_v36_columns.sql"

if [ ! -f "$SQL_FILE" ]; then
    echo "❌ V46 SQL not found at: $SQL_FILE"
    echo "   Did you forget to git pull?"
    exit 1
fi

echo "==> Applying V46 on $PARK_SPACE_DB ..."
...
echo "==> Done. V46 applied to $PARK_SPACE_DB."
