# 云枢中台 + csyh 业务融合实施计划 (v2.0)

> **版本**: v2.0 (2026-06-15 调整)
> **v1.0 → v2.0 变更**: 新增 Phase -1 公共层前置 / 命名空间统一 / 去除资源估算与 MVP 拆解 / 服务拆分保留不合并
> **目标**: 将 csyh 园区运营管理业务能力融入云枢中台，构建"横向中台 + 垂直业务"双层架构
> **执行模式**: AI 重构为主，代码翻译 + 业务逻辑参考
...
| 调整项 | v1.0 | v2.0 | 原因 |
|--------|------|------|------|
| 公共层适配 | 未规划 | **Phase -1 前置 (5 周)** | 7,557 个 cn.flyrise.* import 源码缺失，是真正的技术瓶颈 |
| 命名空间 | 未提及 | **统一为 com.cloudhub.platform.park.*** | 避免双命名空间长期维护成本 |
| MVP 拆解 | 有建议 | **删除** | 按合理计划推进，反馈周期纳入 Phase 5 整合测试 |
| 资源评估 | 有 5.1 章节 | **删除** | AI 重构为主，无人力瓶颈 |
| 服务拆分 | 6 个 park-* 不合并 | **保留 6 个不合并** | 业务边界清晰，独立扩缩容 |
| 服务器配置 | 未提扩容 | **暂忽略** | K8s 化或硬件升级后续单独规划 |
...
| `cn.flyrise.security.*` | 不计 | Shiro + 自研安全框架 | ⭐⭐⭐ |
| `cn.flyrise.business.*` | 713 | 业务通用模块 | ⭐⭐⭐ |
| `cn.flyrise.system.*` | 244 | 系统工具（用户/权限/字典） | ⭐⭐ |
| `cn.flyrise.fe.common.*` | - | 前端 VO/DTO | ⭐ (重设计) |
| `cn.flyrise.mq.*` | - | 消息队列封装 | ⭐⭐ |
| `cn.flyrise.job.*` / `quartz.*` | - | 定时任务 | ⭐ |
| `cn.flyrise.oss.*` | - | 对象存储 | ⭐ (已有 MinIO) |
| `cn.flyrise.redis.*` | - | Redis 封装 | ⭐ (已有 Spring Data Redis) |
| **合计外部依赖** | **7,557** | | |
...
> v2.0 同步云枢最新进度。**P0/P1 增强已就位**，无需重复实现。
...
| 链路追踪 | 各微服务 | 🟡 M6 P1-1 代码就位 | park-* 模块启用 Zipkin 客户端 |
...
| csyh 技术 | 云枢替代 | 工作量 | 状态 |
|-----------|----------|--------|------|
| `cn.flyrise.common.core.domain.Reply` | 云枢 `Result<T>` | 已存在 | 公共层适配 |
| `cn.flyrise.common.core.domain.BizException` | 云枢 `BusinessException` | 已存在 | 公共层适配 |
| `cn.flyrise.mybatis.base.IBaseService` | MyBatis-Plus `IService` | 已存在 | 公共层适配 |
| `cn.flyrise.mybatis.base.BaseEntity` | 云枢 `BaseEntity` | 扩展 | 公共层适配 |
| `cn.flyrise.common.security.utils.SecurityUtils` | 云枢 `SecurityUtils` | 需实现 | 公共层适配 |
| `cn.flyrise.common.core.annotation.Inner` | 云枢 `@Inner` | 需实现 | 公共层适配 |
| `cn.flyrise.pai.fe.common.*` (VO/DTO) | 重新设计 DTO | 各 1-2 天 | 业务模块 |
| `cn.flyrise.common.oss.OssClient` | 云枢 MinIO | 已存在 | 公共层适配 |
| `cn.flyrise.common.redis.service.RedisService` | Spring Data Redis | 已存在 | 公共层适配 |

---

## 六、关键决策

### 6.1 微服务拆分（保留 6 个 park-* 不合并）
...
**总周期**: 21 周 ≈ 5 个月
**缓冲**: 含 Phase -1 5 周公共层 + Phase 5 2 周整合优化，缓冲充足
...
| # | 变更 | 章节 | 原因 |
|---|------|------|------|
| 1 | 新增 Phase -1 公共层适配 | 二 | 7,557 个 cn.flyrise import 源码缺失是真前置 |
| 2 | 新增命名空间统一 | 二.2.3 + 六.6.4 | 避免双命名空间长期维护 |
| 3 | 删除资源估算 | 原 5.1/5.2 | AI 重构无开发人员瓶颈 |
| 4 | 删除 MVP 拆解建议 | - | 按合理计划推进 |
| 5 | 删除服务合并讨论 | - | 服务拆分粒度合理不合并 |
| 6 | 删除服务器配置 | - | K8s 化后续单独规划 |
| 7 | 删除 SSO P0 项 | 四.5.1 | csyh 旧用户体系对接纳入公共层适配 |
| 8 | 简化 P0/P1 增强 | 五 | 云枢已就位，列出可直接复用项 |
| 9 | 21 周周期不变 | 八 | 业务模块总量未变，Phase -1 是新增前置 |
...
_生成日期: 2026-06-15_  
_v2.0 调整基础: 用户 5 项决策 + AI 评审 7 项建议_  
_执行模式: AI 重构 + 公共层前置 + 命名空间统一_
